...
class _EmployeeProfileScreenState extends State<EmployeeProfileScreen> {
  ...
  void _confirmLogout(BuildContext context) {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: champagneGold.withValues(alpha: 0.2))),
        title: const Text('Log out?', style: TextStyle(color: textFrost, fontWeight: FontWeight.bold)),
        content: const Text('You will need to sign in again to access the Employee Portal.', style: TextStyle(color: textMuted, fontSize: 13)),
        actions: [
  ...
  Widget _preferenceSwitch(String label, bool value, ValueChanged<bool> onChanged) {
    return Row(
      children: [
        ...
        Switch(value: value, activeColor: champagneGold, onChanged: onChanged),
      ],
    );
  }
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _preferenceSwitch('Email alerts', _emailAlerts, (val) => setDialogState(() => _emailAlerts = val)),
                  _preferenceSwitch('Push notifications', _pushAlerts, (val) => setDialogState(() => _pushAlerts = val)),
                  _preferenceSwitch('Status update alerts', _statusUpdateAlerts, (val) => setDialogState(() => _statusUpdateAlerts = val)),
                ],
              ),
              actions: [
                TextButton(
  ...
  void _showReimbursementDetails() {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        backgroundColor: darkCharcoal,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: champagneGold.withValues(alpha: 0.2))),
        title: const Text('Reimbursement Details', style: TextStyle(color: textFrost, fontWeight: FontWeight.bold)),
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Got it', style: TextStyle(color: champagneGold, fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
  ...
  void _showHelpSupport() {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Close', style: TextStyle(color: champagneGold, fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            ...
            SizedBox(height: 10),
            Text('A mobile expense submission and approval system for employees and managers.', style: TextStyle(color: textMuted, fontSize: 13)),
            SizedBox(height: 14),
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          width: 64,
                          height: 64,
                          decoration: BoxDecoration(color: darkCharcoal, shape: BoxShape.circle, border: Border.all(color: champagneGold.withValues(alpha: 0.3), width: 1.5)),
                          alignment: Alignment.center,
                          child: Text(
                            store.currentEmployeeName.isNotEmpty ? store.currentEmployeeName[0].toUpperCase() : 'E',
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(store.currentEmployeeName, style: const TextStyle(color: textFrost, fontSize: 19, fontWeight: FontWeight.bold)),
                              const SizedBox(height: 4),
                              Text(store.currentEmployeeEmail, style: const TextStyle(color: textMuted, fontSize: 13)),
                              const SizedBox(height: 6),
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
                                decoration: BoxDecoration(color: champagneGold.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)),
                                child: const Text('EMPLOYEE', style: TextStyle(color: champagneGold, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                              ),
                            ],
                          ),
                        ),
                    ...
                    Row(
                      children: [
                        Expanded(child: _statCard('Pending', store.myPendingCount.toString(), champagneGold)),
                        const SizedBox(width: 12),
                        Expanded(child: _statCard('Approved', store.myApprovedCount.toString(), Colors.greenAccent.shade400)),
                        const SizedBox(width: 12),
                        Expanded(child: _statCard('Rejected', store.myRejectedCount.toString(), Colors.redAccent.shade400)),
                      ],
                    ),
                    const SizedBox(height: 28),
                    ...
                    _settingsGroup([
                      ...
                      _SettingsItemData(Icons.account_balance_wallet_rounded, 'Reimbursement Details', 'How and when you get paid', _showReimbursementDetails),
                    ]),
  ...
  Widget _statCard(String label, String value, Color color) {
    return Container(
      ...
      decoration: BoxDecoration(color: darkCharcoal, borderRadius: BorderRadius.circular(14), border: Border.all(color: champagneGold.withValues(alpha: 0.1))),
      child: Column(
        children: [
  ...
  Widget _settingsGroup(List<_SettingsItemData> items) {
    return Container(
      decoration: BoxDecoration(color: darkCharcoal, borderRadius: BorderRadius.circular(16), border: Border.all(color: champagneGold.withValues(alpha: 0.1))),
      child: Column(
        children: List.generate(items.length, (index) {
          final item = items[index];
          final isLast = index == items.length - 1;
          return Container(
            decoration: BoxDecoration(border: isLast ? null : const Border(bottom: BorderSide(color: Color(0xFF262633), width: 1))),
            child: ListTile(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(child: Text(text, style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 13))),
        ],
      ),
    );
  }
